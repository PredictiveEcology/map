...
#' @export
runBoxPlotsVegCover <- function(map, dt, analysisGroups) {
  browser()

  allRepPolys <- na.omit(map@metadata[[analysisGroups]])
  names(allRepPolys) <- allRepPolys

  # Here is hard coded 1 data.table -- should be in Lapply
  data <- map@analysesData[[dt]][["LeadingVegTypeByAgeClass"]]$`Small Study Area`
  par(mfrow = c(3,4))
  out <- data[, .doPlotBoxplot(data = .SD, authStatus = TRUE, col = "limegreen",
                        horizontal = TRUE,
